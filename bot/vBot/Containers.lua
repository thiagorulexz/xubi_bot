...
macro(5000, function()
    ...
    local bpId = bp:getId()
    if findItemsInArray(config.list, bpId) then return end

    warn("[Backpacks] Sua Backpack principal (id " .. bpId .. ") nao esta configurada")
end)
...
mainLoop = macro(150, function(macro)
    local storageVal = config.list
    for _, entry in pairs(storageVal) do
